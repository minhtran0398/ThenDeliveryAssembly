...
namespace ThenDelivery.Server.Application.OrderController.Queries
{
   public class GetOrderByMerchantId : IRequest<IEnumerable<OrderDto>>
   {
		...
		public class Handler : IRequestHandler<GetOrderByMerchantId, IEnumerable<OrderDto>>
		{
			private readonly ThenDeliveryDbContext _dbContext;
			private readonly ILogger<GetOrderByMerchantId> _logger;

			public Handler(ThenDeliveryDbContext dbContext, ILogger<GetOrderByMerchantId> logger)
			{
				_dbContext = dbContext;
				_logger = logger;
			}

			public async Task<IEnumerable<OrderDto>> Handle(GetOrderByMerchantId request,
